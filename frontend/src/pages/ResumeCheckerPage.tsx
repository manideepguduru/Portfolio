  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <div className={styles.kicker}>Resume Tools</div>
        <h1>Run your ATS resume check inside the portfolio app.</h1>
        <p>
          Upload a resume, add optional role context, and get score, extracted details, issues, and improvement tips at
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          ...
          <label className={`${styles.field} ${styles.fieldWide}`}>
            ...
            <textarea
              rows={6}
              placeholder="Optional: paste a job description or enter a role like Backend Java Developer"
          ...
          <div className={styles.results}>
            <div className={styles.scoreCard}>
              <div className={styles.scoreTop}>
                <div>
                  ...
                  <div className={styles.scoreValue}>{analyzeResult.score}<span>/100</span></div>
                </div>
            ...
            <div className={styles.cardsGrid}>
              <article className={styles.detailCard}>
                ...
                <p><strong>Name:</strong> {analyzeResult.detectedData.candidateName} <ConfidenceTag value={analyzeResult.detectedData.candidateName} /></p>
                <p><strong>Email:</strong> {analyzeResult.detectedData.email} <ConfidenceTag value={analyzeResult.detectedData.email} /></p>
                <p><strong>Phone:</strong> {analyzeResult.detectedData.phone} <ConfidenceTag value={analyzeResult.detectedData.phone} /></p>
                <p><strong>Target Role:</strong> {analyzeResult.detectedData.targetRole} <ConfidenceTag value={analyzeResult.detectedData.targetRole} /></p>
                <p><strong>Top Skills:</strong> {analyzeResult.detectedData.topSkills.join(', ')}</p>
              </article>

              <article className={styles.detailCard}>
                <h3>Strengths</h3>
                <ul>
                  {analyzeResult.strengths.map((item) => <li key={item}>{item}</li>)}
                </ul>
              </article>
              ...
              <article className={styles.detailCard}>
                <h3>Issues Found</h3>
                <ul>
                  {analyzeResult.issuesFound.map((item) => <li key={item}>{item}</li>)}
                </ul>
              </article>
            </div>

            <article className={styles.tipsCard}>
              <h3>Tips to Improve Score</h3>
              <ul>
                {analyzeResult.tips.map((tip) => <li key={tip}>{tip}</li>)}
              </ul>
            </article>
              ...
              <article className={styles.compareCard}>
                ...
                {compareTarget && scoreDelta !== null && (
                  <p className={`${styles.delta} ${scoreDelta >= 0 ? styles.deltaUp : styles.deltaDown}`}>
                    Score change: {scoreDelta >= 0 ? '+' : ''}{scoreDelta}% compared with Run #{compareTarget.runId}
                  </p>
                )}
              </article>
            )}
